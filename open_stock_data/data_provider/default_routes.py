...
def create_default_routes() -> RouteRegistry:
    ...
    routes = [
        ...
        # ---- 市场概览（Akshare 独有；TTL 沿用原 tools 直连时的缓存时长）----
        RouteSpec(
            Operation.MARKET_PE_PERCENTILE,
            None,
            ("AkshareFetcher",),
            "get_market_pe_percentile",
            CachePolicy(43200),
        ),
        RouteSpec(
        ...
        RouteSpec(
            Operation.FINANCIAL_COMPARE,
            None,
            ("AkshareFetcher",),
            "get_financial_compare",
            CachePolicy(86400 * 7),
        ),
        RouteSpec(
        ...
        RouteSpec(
            Operation.SECTOR_FUND_FLOW_RANK,
            None,
            ("AkshareFetcher",),
        ...
        # ---- 指数日线（原 tools 直连 index_zh_a_hist 的迁移）----
        RouteSpec(
            Operation.INDEX_DAILY,
            None,
            ("AkshareFetcher",),
            "get_index_daily",
            CachePolicy(CACHE_TTLS["daily"]),
        ),
    ]
